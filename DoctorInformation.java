...
import pers.cyz.controller.RootAction;
...
public class DoctorInformation extends JFrame {
	...
	public DoctorInformation()  {
		...
		// 设置背景图片
		new BackgroundImage(frame,container,"aaaaa.png");
		// 添加工具栏以及各组件和监听事件
		new MenuBar(frame);
	...
	private void setButtonChange() {
		...
		buttonChange.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					rootAction.changeBookInformation(textFieldISBN, textFieldBookName
	...
	private void setButtonDel() {
		...
		buttonDel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					rootAction.delBookInformation(table);
					frame.setVisible(false);	
					new DoctorInformation();
				} catch(Exception e1) {
					e1.printStackTrace();
		...
		buttonAdd.addActionListener(new ActionListener() {		
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(textFieldISBN.getText().length() == 0) {
					...
					try {
						rootAction.addBookInformation(textFieldISBN, textFieldBookName, textFieldPrice,textFieldAuthor
	...
	private void setTable() {
		String[] columnNames = {"ID", "医生姓名","医生科室","医生专长","所属医院"
				,"科室号","分类号", "患者姓名", "患者咨询"
				};	
		try {
			RootAction rootAction = new RootAction();
			Object[][] results = rootAction.initializTable(columnNames);
